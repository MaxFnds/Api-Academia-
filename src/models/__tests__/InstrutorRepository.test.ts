import fs from "fs";
import { InstrutorRepository } from "../InstrutorRepository";
...
describe("InstrutorRepository", () => {
  const repository = new InstrutorRepository();
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    fsMock.readFileSync.mockReturnValue(JSON.stringify(instrutoresFake) as any);
  ...
  it("deve listar todos os instrutores como instâncias de Instrutor", () => {
    ...
    expect(instrutores[0]).toBeInstanceOf(Instrutor);
  });
  ...
  it("deve criar um novo instrutor", () => {
    const novo = new Instrutor("i3", "Bia Rocha", "bia@email.com", "hash3", "Crossfit");
  ...
  it("deve atualizar um instrutor existente", () => {
    const atualizado = new Instrutor("i1", "Carlos Souza Jr.", "carlos@email.com", "hash1", "Musculação");
